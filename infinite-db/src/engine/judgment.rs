...
pub fn prepare_judgment_assertion(
    ...
    Ok(HypergraphWriteRow::new_row(
        assertion_space,
        judgment_storage_point(record.id),
        encode_judgment(record)?,
        false,
    ))
}

pub fn prepare_judgment_index_row(record: &JudgmentRecord) -> HypergraphWriteRow {
    HypergraphWriteRow::new_row(
        JUDGMENT_INDEX_SPACE,
        judgment_index_point(&record.subject, record.arbiter, record.id),
        encode_judgment_index_payload(record.id),
        false,
    )
}
